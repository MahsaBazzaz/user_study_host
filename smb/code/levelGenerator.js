        ...
        for (let col = 0; col < rows[row].length; col++) {
            ...
            if (char == "X" && y == this.Height - 2){
                ...
                if (rows[row][col-1] != "X"){
                    ...
                    level.SetBlock(x, y, 8 * 16 + 6);
                    level.SetBlock(x, this.Height - 1, 10 * 16 + 6); 
                }
                else {
                    level.SetBlock(x, y, 8 * 16 + 5);
                    level.SetBlock(x, this.Height - 1, 10 * 16 + 5); 
                ...
                level.SetBlock(x, y, 2 * 16 + 0);
            }
            else if (char == "?" || char == "Q"){
                level.SetBlock(x, y, 1 * 16 + 4);
            }
            else if (char == "{"){
                level.SetBlock(x, y - 1, 4 * 16 + 3);
                level.SetBlock(x+1 , y -1,4 * 16 + 4);
                level.SetBlock(x, y, 5 * 16 + 3);
                level.SetBlock(x+1 , y, 5 * 16 + 4);
...
Mario.LevelGenerator.prototype.GetMarioPos = function() {
    const rows = this.Text.split('\n'); // Split by lines
