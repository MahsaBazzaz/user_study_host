...
Mario.LevelState.prototype.Enter = function() {
    ...
    var maiopos = levelGenerator.GetMarioPos()
    // console.log(maiopos[0],maiopos[1])
    ...
    this.Layer = new Mario.LevelRenderer(this.Level, THE_WIDTH, THE_HEIGHT);
    ...
    for (i = 0; i < 2; i++) {
        scrollSpeed = 4 >> i;
        w = ((((this.Level.Width * 16) - THE_WIDTH) / scrollSpeed) | 0) + THE_WIDTH;
        h = ((((this.Level.Height * 16) - THE_HEIGHT) / scrollSpeed) | 0) + THE_HEIGHT;
        bgLevelGenerator = new Mario.BackgroundGenerator(w / 32 + 1, h / 32 + 1, i === 0, this.LevelType);
        this.BgLayer[i] = new Mario.BackgroundRenderer(bgLevelGenerator.CreateLevel(), THE_WIDTH, THE_HEIGHT, scrollSpeed);
    }

    Mario.MarioCharacter.Initialize(this);
    Mario.MarioCharacter.X = maiopos[0] * 16;
    Mario.MarioCharacter.Y = maiopos[1] * 16;
...
Mario.LevelState.prototype.Update = function(delta) {
    ...
    if (this.Camera.X > this.Level.Width * 16 - THE_WIDTH) {
        this.Camera.X = this.Level.Width * 16 - THE_WIDTH;
    }
    ...
    for (i = 0; i < this.Sprites.Objects.length; i++) {
        ...
        if (sprite !== Mario.MarioCharacter) {
            ...
            if (xd < -64 || xd > THE_WIDTH + 64 || yd < -64 || yd > THE_HEIGHT + 64) {
...
Mario.LevelState.prototype.Draw = function(context) {
    ...
    if (this.Camera.Y > this.Level.Height * 16 - THE_HEIGHT) {
        this.Camera.Y = this.Level.Height * 16 - THE_HEIGHT;
    }
    ...
    this.DrawStringShadow(context, "COIN", 0, 0);
    this.DrawStringShadow(context, " " + Mario.MarioCharacter.Coins, 0, 1);
...
Mario.LevelState.prototype.Save = function() {
    ...
    for (let c = 0; c < chunkCount; c++) {
        fullCamera.X = c * patchSize;
        context.save();
        context.translate(fullCamera.X, 0);

        this.BgLayer[0].Draw(context, fullCamera);
        this.BgLayer[1].Draw(context, fullCamera);
        this.Layer.FixDynamic(context, fullCamera);
        
        context.restore();
        this.Layer.DrawExit0(context, this.Camera, false);
        this.Layer.DrawExit1(context, fullCamera);
        context.restore();
    }

   
    var imageDataURL = bufferCanvas.toDataURL("image/png");
    var link = document.createElement("a");
    link.href = imageDataURL;
    link.download = `${this.PageId}.png`;
    link.click();
};
